SELECT i.Name, i.Comment, i.Value as English, ja.Value as Japanese, ja.Comment as JapaneseComment, zh.Value as Chinese, zh.Comment as ChineseComment, (SELECT MIN(FilePath) FROM ResourceLocation loc INNER JOIN ResxFile f ON loc.ResxFileId = f.Id WHERE loc.InvariantResourceId = i.Id) AS FirstFile, (SELECT COUNT(Id) FROM ResourceLocation loc WHERE loc.InvariantResourceId = i.Id) as FileCount
